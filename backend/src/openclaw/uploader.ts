...
export async function uploadSkillZip(
  ...
  try {
    // 3) 找 manifest.json（兼容 zip 根目录 或 单层子目录）
    const manifestInfo = await locateManifest(stageDir);
    ...
    catch { throw new Error(`找不到入口脚本：${manifest.entry}`); }
    ...
    try {
      ...
      if (existing.uploadedBy && existing.uploadedBy !== tenant.userId) {
        throw new Error(`技能 ${manifest.id} 已被其他用户上传，请改个 id`);
      }
    } catch (e: any) {
...
async function locateManifest(root: string): Promise<{ manifestPath: string; root: string }> {
  // 顶层有 manifest.json？
  try {
  ...
  throw new Error('zip 内未找到 manifest.json（请放在根目录或单层子目录）');
}
